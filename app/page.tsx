...
export default function HomePage() {
  ...
  const handleFinalCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) alert(`Order submission failed.`);
  ...
  const filteredProducts = products.filter((p) => {
    ...
    // THE ETHNIC FIX: Partial match brand search
    const brandName = (p.shop?.name || "").toLowerCase();
    const filterName = selectedBrand.toLowerCase().split(' ')[0];
    const matchesBrand = selectedBrand === 'All' || brandName.includes(filterName);
    ...
    <div className="min-h-screen bg-[#FAF8F5] text-[#1A1816] font-sans selection:bg-[#C8521B] selection:text-white relative overflow-x-hidden pb-20 md:pb-0">
      ...
      {showPromoBanner && (
        <div className="bg-[#C8521B] text-white text-[11px] font-bold py-2.5 px-4 text-center relative flex items-center justify-center gap-2 shadow-sm z-50">
          ...
          <button onClick={() => setShowPromoBanner(false)} className="absolute right-4 text-white/80 hover:text-white">✕</button>
        </div>
      )}
      ...
      <nav className="bg-[#FAF8F5]/90 backdrop-blur-2xl border-b border-[#E6E2D8] sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-8 py-4 flex justify-between items-center gap-6">
          ...
          <div className="flex-1 max-w-md relative hidden md:block">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search Khaadi, Outfitters, Ethnic..." className="w-full pl-11 pr-4 py-2.5 bg-white border border-[#E6E2D8] rounded-full text-xs" />
          </div>
          <div className="flex items-center gap-4 text-xs font-bold tracking-wider uppercase">
            <button onClick={() => setShowVendorModal(true)} className="hidden lg:flex items-center gap-1.5 px-4 py-2 bg-emerald-50 text-emerald-900 rounded-full border border-emerald-200 transition-all text-[11px]"><UserPlus className="w-3.5 h-3.5" /> <span>Become a Seller</span></button>
            ...
            <button onClick={() => setShowCart(true)} className="relative p-2.5 bg-[#C8521B] text-white rounded-full shadow-md shadow-[#C8521B]/20"><ShoppingBag className="w-4 h-4" />{cart.length > 0 && <span className="absolute -top-1.5 -right-1.5 bg-[#1A1816] text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center">{cart.length}</span>}</button>
          </div>
        </div>
      </nav>

      {/* RESTORED ATRIUM HERO SECTION */}
      <div className="relative py-20 px-8 text-center border-b border-[#E6E2D8] overflow-hidden bg-gray-950">
        ...
        <div className="relative z-10 max-w-5xl mx-auto space-y-6">
          ...
          <p className="text-gray-200 text-sm sm:text-lg max-w-2xl mx-auto font-light leading-relaxed">Shop official inventory from premium labels directly in one single cart.</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8 py-16">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 border-b border-[#E6E2D8] pb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((cat) => (<button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-5 py-2 rounded-full text-xs font-bold border transition-all ${selectedCategory === cat ? 'bg-[#1A1816] text-white' : 'bg-white text-[#66615C] border-[#E6E2D8]'}`}>{cat}</button>))}
          </div>
        </div>
            ...
            return (
              <div key={product.id} onClick={() => handleOpenProduct(product)} onMouseEnter={() => setHoveredProductId(product.id)} onMouseLeave={() => setHoveredProductId(null)} className="bg-white border border-[#E6E2D8] rounded-3xl overflow-hidden shadow-sm hover:shadow-xl hover:border-[#C8521B] transition-all duration-300 group cursor-pointer flex flex-col justify-between">
                <div className="relative h-80 w-full overflow-hidden bg-gray-100">
                  ...
                  <div className="absolute top-3 left-3 bg-white/90 px-3 py-1.5 rounded-full border border-[#E6E2D8] text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5"><Store className="w-3 h-3 text-[#C8521B]" /> {product.shop?.name}</div>
                  ...
                  {images.length > 1 && <div className="absolute bottom-3 right-3 bg-black/70 text-white px-2 py-1 rounded-lg text-[9px] font-bold">+{images.length - 1} View</div>}
                </div>
                <div className="p-6">
                  ...
                  <div className="pt-4 border-t mt-4 flex items-center justify-between">
                    ...
                    <button className="px-5 py-2.5 bg-[#C8521B] text-white font-bold text-xs rounded-full shadow-md transition-all">Select Size</button>
                  </div>
                </div>
              </div>
            );
      ...
      {/* QUICK VIEW MODAL - GALLERY & SIZING RESTORED */}
      {selectedProduct && (
          ...
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl text-xs space-y-4 relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleFinalCheckout} className="space-y-3.5">
              ...
              <div className="space-y-2">
                <button type="button" onClick={() => setPaymentMethod('PREPAID')} className={`w-full p-3 rounded-2xl border text-left flex justify-between ${paymentMethod === 'PREPAID' ? 'bg-emerald-50 border-emerald-500 text-emerald-900' : 'bg-gray-50 text-gray-600'}`}><div className="flex items-center gap-3"><CreditCard className="w-4 h-4" /><div><p className="font-bold text-xs">Prepaid Order (JazzCash / Bank)</p><p className="text-[9px]">Free Delivery across Pakistan</p></div></div><span className="text-[9px] bg-emerald-600 text-white font-black px-2 py-0.5 rounded">FREE</span></button>
                <button type="button" onClick={() => setPaymentMethod('COD')} className={`w-full p-3 rounded-2xl border text-left flex justify-between ${paymentMethod === 'COD' ? 'bg-amber-50 border-[#C8521B] text-[#1A1816]' : 'bg-gray-50 text-gray-600'}`}><div className="flex items-center gap-3"><Truck className="w-4 h-4" /><div><p className="font-bold text-xs">Standard Cash on Delivery</p><p className="text-[9px]">2-3 Days Delivery</p></div></div><span className="text-xs font-bold">PKR 195</span></button>
                <button type="button" onClick={() => setPaymentMethod('EXPRESS_COD')} className={`w-full p-3 rounded-2xl border text-left flex justify-between ${paymentMethod === 'EXPRESS_COD' ? 'bg-amber-100 border-amber-500 text-amber-950 shadow-sm' : 'bg-gray-50 text-gray-600'}`}><div className="flex items-center gap-3"><Clock className="w-4 h-4" /><div><p className="font-bold text-xs">VIP 24-Hour COD (Lahore)</p><p className="text-[9px]">Guaranteed Same-Day Rider</p></div></div><span className="text-xs font-bold">PKR 350</span></button>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-[#E6E2D8] rounded-3xl p-8 max-w-lg w-full shadow-2xl text-xs space-y-6 relative max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start border-b pb-4">
              <div><span className="bg-emerald-100 text-emerald-800 text-[10px] font-bold px-3 py-1 rounded-full uppercase">Order Submitted ✓</span><h3 className="text-2xl font-serif text-[#1A1816] mt-2">Order Invoice Receipt</h3><p className="text-[#66615C]">Order ID: {lastOrder.orderId} • {lastOrder.date}</p></div>
              <button onClick={() => setShowInvoice(false)} className="text-gray-400 font-bold text-base">✕</button>
            </div>
            ...
            <div className="space-y-3">
              {Object.values(lastOrder.shopBreakdown).map((sub: any, idx: number) => (
                <div key={idx} className="bg-gray-50 border p-4 rounded-2xl space-y-2">
                  <div className="flex justify-between font-bold text-[#1A1816] border-b pb-2"><span className="text-[#C8521B]">{sub.shopName}</span><span>Subtotal: PKR {sub.subtotal.toLocaleString()}</span></div>
