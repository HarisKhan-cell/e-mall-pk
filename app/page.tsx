...
export default function HomePage() {
  ...
  const handleFinalCheckout = async (e: React.FormEvent) => {
    ...
    cart.forEach((item) => {
      ...
      if (isResale) {
        // Haris Reselling: Add 5% extra to customer price
        itemMarkup = (item.price * item.quantity * 5) / 100;
        itemProfit = itemMarkup;
      } else {
        // Third-Party Seller: Take 10% commission from seller (customer pays normal)
        itemMarkup = 0;
      ...
      // FIXED INITIALIZATION LOGIC (No more PKR 0)
  ...
  const brands = [
    { name: 'ALL BRANDS', filter: 'All' }, { name: 'KHAADI', filter: 'Khaadi Official' },
    { name: 'OUTFITTERS', filter: 'Outfitters Official' }, { name: 'ETHNIC', filter: 'Ethnic Official' },
    { name: 'BREAKOUT', filter: 'Breakout Official' }, { name: 'LAMA', filter: 'LAMA' },
    { name: 'AGHA NOOR', filter: 'Agha Noor Official' }, { name: 'SAPPHIRE', filter: 'Sapphire Official' },
  ];
  ...
  const filteredProducts = products.filter((p) => {
    ...
    const matchesBrand = selectedBrand === 'All' || brandName.includes(filterKey);
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-[#1A1816] font-sans selection:bg-[#C8521B] relative overflow-x-hidden pb-20 md:pb-0">
      ...
      <nav className="bg-[#FAF8F5]/90 backdrop-blur-2xl border-b border-[#E6E2D8] sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-8 py-4 flex justify-between items-center gap-6">
          ...
          <div className="flex items-center gap-4 text-xs font-bold tracking-wider uppercase">
            <Link href="/track-order" className="hidden lg:flex items-center gap-1.5 px-4 py-2 bg-white text-[#1A1816] rounded-full border border-[#E6E2D8] text-[11px] shadow-sm hover:bg-gray-50 transition-all"><PackageCheck className="w-3.5 h-3.5 text-[#C8521B]" /><span>Track Order</span></Link>
      ...
      {/* FILTER BAR */}
      <div className="border-b border-[#E6E2D8] bg-[#F5F2EB] py-4 flex items-center justify-center gap-3 flex-wrap px-4">
        {brands.map((b, idx) => (
          <button key={idx} onClick={() => setSelectedBrand(b.filter)} className={`px-5 py-2 rounded-full text-xs transition-all border ${selectedBrand === b.filter ? 'bg-[#C8521B] text-white border-[#C8521B] shadow-md font-bold' : 'bg-white text-[#66615C] border-[#E6E2D8] hover:text-[#1A1816]'}`}>{b.name}</button>
        ))}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8 py-16">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 border-b border-[#E6E2D8] pb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((cat) => (<button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-5 py-2 rounded-full text-xs font-bold border transition-all ${selectedCategory === cat ? 'bg-[#1A1816] text-white border-[#1A1816]' : 'bg-white text-[#66615C] border-[#E6E2D8] hover:text-[#1A1816]'}`}>{cat}</button>))}
          </div>
        </div>
            ...
            return (
              <div key={product.id} onClick={() => handleOpenProduct(product)} onMouseEnter={() => setHoveredProductId(product.id)} onMouseLeave={() => setHoveredProductId(null)} className="bg-white border border-[#E6E2D8] rounded-3xl overflow-hidden shadow-sm hover:shadow-xl hover:border-[#C8521B] transition-all duration-300 group cursor-pointer flex flex-col justify-between">
                <div className="relative h-80 w-full overflow-hidden bg-gray-100">
                  ...
                  <div className="absolute top-3 right-3"><div className="bg-emerald-50 text-emerald-700 backdrop-blur-md px-2.5 py-1 rounded-full border border-emerald-200 text-[9px] font-bold flex items-center gap-1.5 shadow-sm"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" /> In Stock</div></div>
                  {images.length > 1 && <div className="absolute bottom-3 right-3 bg-black/70 text-white px-2 py-1 rounded-lg text-[9px] font-bold uppercase tracking-widest">+{images.length - 1} Photos</div>}
                </div>
                <div className="p-6">
          ...
          <div className="bg-white rounded-3xl p-8 max-w-4xl w-full shadow-2xl relative overflow-y-auto max-h-[90vh]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-5">
                <div><h3 className="text-2xl font-serif text-[#1A1816] mt-1">{selectedProduct.title}</h3><p className="text-[#66615C] text-xs leading-relaxed mt-2">{selectedProduct.description}</p></div>
                ...
                <button onClick={handleAddToCartFromModal} className="w-full py-4 bg-[#C8521B] text-white font-extrabold text-xs rounded-full shadow-lg flex items-center justify-center gap-2 transition-all hover:bg-[#a83e10] uppercase tracking-widest"><ShoppingBag className="w-4 h-4" /> <span>Add to Cart (PKR {((selectedProduct.price * selectedQty) + (isGiftWrap ? 150 : 0)).toLocaleString()})</span></button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showCart && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-end z-50">
          <div className="bg-white border-l border-[#E6E2D8] w-full max-w-md h-full p-8 shadow-2xl flex flex-col justify-between overflow-y-auto">
            <div>
              <div className="flex justify-between items-center pb-5 border-b border-[#E6E2D8]"><div className="flex items-center gap-2"><ShoppingBag className="w-5 h-5 text-[#C8521B]" /><h3 className="text-lg font-bold text-[#1A1816] font-serif">Your Multi-Brand Cart</h3></div><button onClick={() => setShowCart(false)} className="p-1 text-gray-400 font-bold font-serif">✕</button></div>
                ...
                <div className="mt-6 space-y-3">
                  {cart.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl border border-[#E6E2D8]">
                      ...
                      <div className="flex items-center gap-3"><span className="text-sm font-extrabold text-[#1A1816]">PKR {item.totalPrice.toLocaleString()}</span><button onClick={() => removeFromCart(idx)} className="text-xs text-red-500 font-bold font-serif">✕</button></div>
                    </div>
                  ))}
                </div>
              )}
            </div>
            {cart.length > 0 && <div className="border-t pt-6 mt-6 space-y-3 text-xs text-[#66615C]"><div className="flex justify-between"><span>Subtotal:</span><span className="font-bold text-[#1A1816]">PKR {calculateSubtotal().toLocaleString()}</span></div><button onClick={() => { setShowCart(false); setShowCheckoutForm(true); }} className="w-full py-4 bg-[#C8521B] hover:bg-[#a83e10] text-white font-extrabold text-sm rounded-full shadow-lg flex items-center justify-center gap-2 mt-4 uppercase tracking-widest"><span>Proceed to Checkout</span><ArrowRight className="w-4 h-4" /></button></div>}
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl text-xs space-y-4 relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleFinalCheckout} className="space-y-3.5">
              ...
              <input type="tel" required value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="WhatsApp Contact" className="w-full px-4 py-3 bg-gray-50 border border-[#E6E2D8] rounded-2xl text-xs text-[#1A1816]" />
              <div className="space-y-2">
                <button type="button" onClick={() => setPaymentMethod('PREPAID')} className={`w-full p-3 rounded-2xl border text-left flex justify-between items-center ${paymentMethod === 'PREPAID' ? 'bg-emerald-50 border-emerald-500 shadow-sm' : 'bg-gray-50 text-gray-600'}`}><div className="flex items-center gap-3"><CreditCard className="w-4 h-4" /><div><p className="font-bold text-xs">Prepaid Order (JazzCash / Bank)</p><p className="text-[9px]">Free Delivery across Pakistan</p></div></div><span className="text-[9px] bg-emerald-600 text-white font-black px-2 py-0.5 rounded">FREE</span></button>
                <button type="button" onClick={() => setPaymentMethod('COD')} className={`w-full p-3 rounded-2xl border text-left flex justify-between items-center ${paymentMethod === 'COD' ? 'bg-amber-50 border-[#C8521B] shadow-sm' : 'bg-gray-50 text-gray-600'}`}><div className="flex items-center gap-3"><Truck className="w-4 h-4" /><div><p className="font-bold text-xs">Standard COD</p><p className="text-[9px]">2-3 Days Delivery</p></div></div><span className="text-xs font-bold">PKR 195</span></button>
                <button type="button" onClick={() => setPaymentMethod('EXPRESS_COD')} className={`w-full p-3 rounded-2xl border text-left flex justify-between items-center ${paymentMethod === 'EXPRESS_COD' ? 'bg-amber-100 border-amber-500 shadow-sm' : 'bg-gray-50 text-gray-600'}`}><div className="flex items-center gap-3"><Clock className="w-4 h-4" /><div><p className="font-bold text-xs">VIP 24-Hour COD (Lahore Only)</p><p className="text-[9px]">Guaranteed Same-Day Dispatch</p></div></div><span className="text-xs font-bold text-amber-900">PKR 350</span></button>
              </div>
              ...
              <button type="submit" className="w-full py-4 bg-[#C8521B] text-white font-extrabold text-sm rounded-full shadow-lg transition-all uppercase tracking-widest">Confirm & Submit Order</button>
            </form>
          </div>
        </div>
      )}

      {/* FULL RESTORED INVOICE - SCENARIO LOGIC FIXED */}
      {showInvoice && lastOrder && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl text-xs space-y-6 relative max-h-[90vh] overflow-y-auto border border-[#E6E2D8]">
                ...
                <div key={idx} className="bg-gray-50 border p-4 rounded-2xl space-y-3">
                  ...
                  {sub.items.map((item: any, i: number) => (
                    <div key={i} className="flex justify-between text-[11px] font-sans">
                       ...
                       <span className="font-bold text-[#1A1816]">PKR {item.totalPrice.toLocaleString()}</span>
                    </div>
                  ))}
            ...
            <div className="border-t pt-4 space-y-2 font-sans border-dashed">
      ...
      <footer className="bg-[#1A1816] border-t border-black text-xs text-[#D8D3CC] font-sans mt-28 pt-16 pb-12 px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 pb-12 border-b border-white/10">
          <div className="space-y-3"><div className="flex items-center gap-3 text-white font-serif font-bold text-sm"><EMallLogoSymbol /><span>E-MALL PAKISTAN</span></div><p className="text-gray-400 text-[11px] leading-relaxed">Pakistan's 1st consolidated digital mall platform sourcing 100% original labels.</p><div className="pt-2"><span className="bg-emerald-950/80 text-emerald-300 border border-emerald-500/30 text-[9px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">✓ 100% Original Brand Guarantee</span></div></div>
          ...
          <div className="space-y-3">
             <h4 className="text-white font-bold uppercase tracking-widest text-[11px] font-serif">Support Hub</h4>
             <div className="space-y-2 text-gray-400 text-[11px]">
               <p className="flex items-center gap-2 text-white font-bold"><Mail className="w-3.5 h-3.5 text-[#C8521B]" /><span>emallofficials869@gmail.com</span></p>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-[#E6E2D8] rounded-3xl p-8 max-w-md w-full shadow-2xl text-xs space-y-4 relative">
            <div className="flex justify-between items-center border-b pb-4">
              ...
              <button onClick={() => setShowVendorModal(false)} className="text-gray-400 font-bold font-serif font-sans">✕</button>
            </div>
            {vendorSuccess ? (
              ...
              <form onSubmit={handleVendorSubmit} className="space-y-3.5 font-sans">
